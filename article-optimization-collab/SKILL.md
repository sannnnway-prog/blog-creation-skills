...
- Only GlobalGPT/GLBGPT homepage and model-page conversion links require tracking parameters. Ordinary editorial/internal article links, especially `/hub/...` blog links, do not get `inviter` or `login=1`.
- GlobalGPT homepage, model homepage, model CTA, model image link, model button, model table link, model FAQ/schema link, model metadata note, and model workbench link must include tracking parameters. Bare `https://www.glbgpt.com`, `https://www.glbgpt.com/home`, and `/home/<model-slug>` conversion links fail QA.
- Use `https://www.glbgpt.com/home?inviter=hub_popup&login=1` for the general GlobalGPT/GLBGPT homepage in hub/blog article copy unless a more specific campaign URL is provided.
- Every GlobalGPT model homepage link in public article copy, image links, buttons, tables, FAQ, schema, metadata notes, and the workbench must use the tracked homepage URL format: `/home/<model-slug>?inviter=hub_content_<modelid>&login=1`. For example, use `https://www.glbgpt.com/home/gpt-5-5?inviter=hub_content_gpt55&login=1`, not a bare `/home/gpt-5-5` URL.
- If a GlobalGPT URL already has query parameters, append `&inviter=...&login=1`; if it has a hash, put the query before the hash.
- Preserve existing non-model product/order CTA tracking only when the user explicitly provides it. Do not add tracking to `/hub/...` article links.
- GPT-5.6 Sol uses this verified tracked URL: `https://www.glbgpt.com/home/gpt-5-6-sol?inviter=hub_content_gptsol56&login=1`. Do not infer or use `hub_content_gpt56sol`. When replacing stale GPT-5.2 contextual planning/research article links, prefer verified GPT-5.6 article pages such as `https://www.glbgpt.com/hub/gpt-5-6-pricing/` and `https://www.glbgpt.com/hub/gpt-5-6-vs-fable-5-vs-gpt-5-5/` where the paragraph needs pricing or comparison context; use the tracked product URL for direct model-entry CTAs.
...
- Anchor text must be semantically relevant to the linked page's actual topic. Before inserting a GLBGPT internal link, verify the destination by sitemap title, live title, or page content, then choose anchor text that names or clearly describes that destination. Generic anchors such as `view the details`, `click here`, `read more`, `this guide`, `learn more`, or `check it out` fail QA unless the surrounding linked words themselves identify the target topic.
- Do not hide a comparison, pricing, access, limits, prompt, or workflow page behind vague trailing text. For example, link `Flux 2 vs Nano Banana Pro comparison`, not `view the details`, when the destination is a Flux 2 vs Nano Banana Pro article.
...
- Convert every GlobalGPT homepage and model homepage URL to a tracked format everywhere it appears in the article package, including text links, image links, buttons, comparison tables, FAQ/schema, metadata notes, and workbench link tables. The general homepage uses `https://www.glbgpt.com/home?inviter=hub_popup&login=1`; model homepages use `/home/<model-slug>?inviter=hub_content_<modelid>&login=1`. Bare homepage/model links such as `https://www.glbgpt.com`, `/home`, or `/home/claude-opus-4-8` fail QA even when the route itself works. Do not add tracking to `/hub/...` blog/article internal links.
...
- Publish QA must fail if model text, anchor text, destination URL, schema, or tracking parameters disagree with one another, if any GlobalGPT homepage/model conversion link is missing `inviter` and `login=1`, or if any `/hub/...` article link has unnecessary `inviter` tracking.
...
Use anchors that sound like normal article text. Do not force exact-match anchors when a softer phrase reads better, but the softer phrase still must identify the target page's real topic. Vague anchors such as `view the details`, `click here`, `read more`, `this guide`, `learn more`, and `check it out` fail internal-link QA when they do not describe the destination. The linked anchor and surrounding sentence must make it obvious why the reader should open that specific page.
